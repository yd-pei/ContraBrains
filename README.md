...
__For videos results, check__ `./evaluation_videos`.
...
-|-
Moving forward reward|$\Delta x_t = x_{\text{scroll},t} - x_{\text{scroll},t-1} - 0.01$ <br> $r_{\text{move},t} = \text{clip}\bigl(\Delta x_t,\,-3,\,3\bigr)$
Killing reward| $\Delta s_t = s_t - s_{t-1}$ <br> $r_{\text{score},t} = \text{clip}\bigl(\Delta s_t,\,0,\,2\bigr)$
Dead penalty|$r_{\text{life},t} = \text{−15 if } \ell_t < \ell_{t-1},\quad \text{0 otherwise}$
Win reward| $r_{\text{end},t} = +50 \quad \text{if } \ell_t > 0 \ (\text{win}); \quad -35 \quad \text{if } \ell_t = 0 \ (\text{lose})$
Instant rewards | $r_t^{\text{raw}} = r_{\text{move},t} + r_{\text{score},t} + r_{\text{life},t} + r_{\text{end},t}$
...
The PPO policy objective is given by: $L^{\text{CLIP}}(\theta)=\mathbb{E}_t\Bigl[\min\bigl(r_t(\theta)A_t,\;\text{clip}(r_t(\theta),\,1-\epsilon,\,1+\epsilon)A_t\bigr)\Bigr]$
